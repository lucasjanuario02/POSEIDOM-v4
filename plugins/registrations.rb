...
def meta_linspace_logspace(start, end, steps, base: nil, dtype: nil, device: nil, layout: Torch.strided, pin_memory: false, requires_grad: false)
  if start.is_a?(Torch::Tensor)
    raise "linspace only supports 0-dimensional start and end tensors" if start.dim != 0
  end

  if end.is_a?(Torch::Tensor)
    raise "linspace only supports 0-dimensional start and end tensors" if end.dim != 0
  end

  dtype ||= Torch.get_default_dtype
  dtype = Torch::DType.new(dtype) unless dtype.is_a?(Torch::DType)
  raise ArgumentError, "dtype must be a Torch::DType" unless dtype.is_a?(Torch::DType)

  raise ArgumentError, "number of steps must be non-negative" if steps < 0

  Torch.empty([steps], dtype: dtype, layout: layout, device: device, pin_memory: pin_memory, requires_grad: requires_grad)
end

def meta_take(self, index)
  raise "take(): Expected a long tensor for index, but got #{index.dtype}" unless index.dtype == :long
  raise "take(): tried to take from an empty tensor" if self.numel == 0 && index.numel != 0
  self.new_empty(index.shape)
end

def linalg_cross(self, other, dim: -1)
  x_d = self.ndim
  y_d = other.ndim
  raise "linalg.cross: inputs must have the same number of dimensions." unless x_d == y_d
  unless self.size(dim) == 3 && other.size(dim) == 3
    raise "linalg.cross: inputs dimension #{dim} must have length 3. Got #{self.size(dim)} and #{other.size(dim)}"
  end
  out_shape = Torch._broadcast_shapes(self.shape, other.shape)
  self.new_empty(out_shape)
end

def linalg_matrix_exp(self)
  squareCheckInputs(self, "linalg.matrix_exp")
  checkFloatingOrComplex(self, "linalg.matrix_exp")
  self.new_empty_like(memory_format: :contiguous)
end

def cummaxmin(self, dim)
  values = Torch.empty(self.shape, device: self.device, dtype: self.dtype)
  indices = Torch.empty(self.shape, device: self.device, dtype: :int64)
  if self.numel != 0 && self.ndim != 0
...
def _exec_fft(out, self, out_sizes, dim, forward)
  ndim = self.ndim
  signal_ndim = dim.size
  batch_dims = ndim - signal_ndim

  dim_permute = (0...ndim).to_a

  is_transformed_dim = [false] * ndim
  dim.each { |d| is_transformed_dim[d] = true }

  left, right = dim_permute.partition { |d| !is_transformed_dim[d] }
  left += right
  batch_end = left.size

  self_strides = self.stride
  tmp = left.take(batch_end).sort_by { |d| -self_strides[d] }
  dim_permute = tmp + left.drop(batch_end)

  input = self.permute(dim_permute)

  batched_sizes = [-1] + input.shape[batch_dims..-1]
  input = input.reshape(batched_sizes)

  batch_size = input.size(0)
  batched_out_sizes = batched_sizes.dup
  dim.each_with_index { |d, i| batched_out_sizes[i + 1] = out_sizes[d] }
  ...
  (batch_dims...ndim).each { |i| out_strides[dim_permute[i]] = out.stride(1 + (i - batch_dims)) }

  out.as_strided(out_sizes, out_strides, out.storage_offset)
end

@register_meta([aten._fft_c2c.default, aten._fft_c2c.out])
@out_wrapper()
def meta_fft_c2c(self, dim, normalization, forward)
  raise "Assert self.dtype.is_complex" unless self.dtype.is_complex

  out_sizes = self.shape
  output = self.new_empty(out_sizes)

  return output unless dim

  sorted_dims = dim.dup
  self_strides = self.stride
  sorted_dims.sort_by! { |x| -self_strides[x] }
  _exec_fft(output, self, out_sizes, sorted_dims, forward)
end

@register_meta([aten._fft_r2c.default, aten._fft_r2c.out])
@out_wrapper()
def meta_fft_r2c(self, dim, normalization, onesided)
  raise "Assert self.dtype.is_floating_point" unless self.dtype.is_floating_point
  output_sizes = self.size.dup
  ...
  self.new_empty(output_sizes, dtype: Torch.corresponding_complex_dtype(self.dtype))
end

@register_meta(aten.randperm.generator_out)
def meta_randperm(n, generator: nil, out:)
  _maybe_resize_out(out, [n])
end

@register_meta(aten.randperm.default)
def meta_randperm_default(n, dtype: :long, layout: nil, device: nil, pin_memory: nil)
  Torch.empty(n, dtype: dtype, layout: layout, device: device, pin_memory: pin_memory)
end

@register_meta([aten.randint.default, aten.randint.out])
@out_wrapper()
def meta_randint(high, size, dtype: :long, layout: nil, device: nil, pin_memory: nil)
  Torch.empty(size, dtype: dtype, layout: layout, device: device, pin_memory: pin_memory)
end

@register_meta([aten.randint.low, aten.randint.low_out])
@out_wrapper()
def meta_randint_low(low, high, size, dtype: :long, layout: nil, device: nil, pin_memory: nil)
  Torch.empty(size, dtype: dtype, layout: layout, device: device, pin_memory: pin_memory)
end

@register_meta([aten.rand.default, aten.rand.out])
@out_wrapper()
def meta_rand_default(size, dtype: nil, layout: nil, device: nil, pin_memory: nil)
  Torch.empty(size, dtype: dtype, layout: layout, device: device, pin_memory: pin_memory)
end
...
def meta_fft_c2r(self, dim, normalization, lastdim)
  ...
  self.new_empty(output_sizes, dtype: toRealValueType(self.dtype))
end

@register_meta(aten.copy_.default)
def meta_copy_(self, src, non_blocking: false)
  raise "This code simulates the original decomp from inductor," +
...
def inferUnsqueezeGeometry(tensor, dim)
  result_sizes = tensor.size.dup
  result_strides = tensor.stride.dup
  new_stride = dim >= tensor.dim ? 1 : result_sizes[dim] * result_strides[dim]
  result_sizes.insert(dim, 1)
  result_strides.insert(dim, new_stride)
  [result_sizes, result_strides]
end
...
def meta_unsqueeze_(self, dim)
  ...
  self.as_strided!(g_sizes, g_strides)
  self
end

@register_meta(aten._sparse_semi_structured_linear)
def meta_sparse_structured_linear(input: Tensor, weight: Tensor, _meta: Tensor, bias: nil, _activation_opt: nil, out_dtype: nil)
  output_sizes = input.shape.dup
  raise "output size mismatch" unless bias.nil? || weight.size(0) == bias.size(0)
  raise "only input broadcasted to columns of mat1 * mat2 product is supported" unless weight.size(1) == input.size(-1) / 2

  transposed_strides = [1, input.size(0)]  # see GitHub issue #114477
  output = input.new_empty(output_sizes, dtype: input.dtype)
  output.as_strided!(output_sizes, transposed_strides)
  output
end

@register_meta(aten._sparse_semi_structured_mm)
def meta_sparse_structured_mm(mat1: Tensor, mat1_meta: Tensor, mat2: Tensor, out_dtype: nil)
  raise "only input broadcasted to columns of mat1 * mat2 product is supported" unless mat1.shape.size == 2
  raise "out_dtype is only supported for i8i8->i32 linear operator" unless out_dtype.nil? || (mat2.dtype == :int8 && out_dtype == :int32)

  output = mat2.new_empty([mat1.size(0), mat2.size(1)], dtype: mat2.dtype)
  output
end

@register_meta(aten._sparse_semi_structured_addmm)
def meta_sparse_structured_addmm(input: Tensor, mat1: Tensor, mat1_meta: Tensor, mat2: Tensor, alpha: 1, beta: 1, out_dtype: nil)
  raise "only input broadcasted to columns of mat1 * mat2 product is supported" unless input.shape.size == 1
  raise "out_dtype is only supported for i8i8->i32 linear operator" unless out_dtype.nil? || (mat2.dtype == :int8 && out_dtype == :int32)
...
def meta_index_reduce(
  ...
  Torch.empty_like(self, memory_format: torch.contiguous_format)
end
...
def meta_index_select(self, dim, index)
  result_size = self.size.dup
  result_size[dim] = index.numel
  self.new_empty(result_size)
end
...
def meta_segment_reduce(
  ...
  if offsets
    lengths_shape = offsets.shape[0...-1] + [offsets.shape[-1] - 1]
...
def meta_max_dim(self, dim, keepdim: false)
  ...
  [self.new_empty(output_shape), self.new_empty(output_shape, dtype: torch.long)]
end
...
def meta_angle(self)
  result_dtype = self.is_complex ? corresponding_real_dtype(self.dtype) : elementwise_dtypes(self, type_promotion_kind: ELEMENTWISE_TYPE_PROMOTION_KIND.INT_TO_FLOAT)[1]
  Torch.empty_like(self, dtype: result_dtype)
end

@register_meta(aten.angle.out)
def meta_angle_out(self, out)
  torch._resize_output_(out, self.size, self.device)
  out.copy_(torch.angle(self))
end
...
def sym_constrain_range(size, min: nil, max: nil)
  raise ValueError("Constraining SymFloat or Symbool is nyi") if size.is_a?(SymFloat) || size.is_a?(SymBool)

@register_meta(aten._functional_sym_constrain_range.default)
def functional_sym_constrain_range(size, min: nil, max: nil, dep_token)
  aten.sym_constrain_range(size, min: min, max: max)
  dep_token
end

@register_meta(aten.sym_constrain_range_for_size.default)
def sym_constrain_range_for_size(size, min: nil, max: nil)
  raise ValueError("Constraining SymFloat or Symbool is nyi") if size.is_a?(SymFloat) || size.is_a?(SymBool)
end
...
def squareCheckInputs(self: Tensor, f_name: str)
  raise "#{f_name}: The input tensor must have at least 2 dimensions." unless self.dim >= 2
  raise "#{f_name}: A must be batches of square matrices, but they are #{self.size[-2]} by #{self.size[-1]} matrices" unless self.size[-1] == self.size[-2]
end
...
def linearSolveCheckInputs(
  ...
  raise "Expected b and A to be on the same device, but found b on #{self.device} and A on #{A.device} instead." unless self.device == A.device
  raise "Expected b and A to have the same dtype, but found b of type #{self.dtype} and A of type #{A.dtype} instead." unless self.dtype == A.dtype
  raise "A must be batches of square matrices, but they are #{A.size[-2]} by #{A.size[-1]} matrices" unless A.size[-1] == A.size[-2]
  raise "Incompatible matrix sizes for #{name}: each A matrix is #{A.size[-1]} by #{A.size[-1]} but each b matrix is #{self.size[-2]} by #{self.size[-1]}" unless A.size[-1] == self.size[-2]
end
...
def checkFloatingOrComplex(
  ...
  dtype = t.dtype
  raise "#{f_name}: Expected a floating point or complex tensor as input. Got #{dtype}" unless t.is_floating_point || t.is_complex

  unless allow_low_precision_dtypes
    raise "#{f_name}: Low precision dtypes not supported. Got #{dtype}" unless [torch.float, torch.double, torch.cfloat, torch.cdouble].include?(dtype)
  end
end

def checkIsMatrix(A: Tensor, f_name: str, arg_name: str = "A")
  raise "#{f_name}: The input tensor #{arg_name} must have at least 2 dimensions." unless A.dim >= 2
end

def checkSameDevice(
    fn_name: str, result: Tensor, input: Tensor, result_name: str = "result"
)
  raise "#{fn_name}: Expected #{result_name} and input tensors to be on the same device, but got #{result_name} on #{
@register_meta(aten.index_reduce.default)
def meta_index_reduce(
    self: Tensor,
    dim: int,
...
def linalg_inv_ex_meta(A: Tensor, check_errors: bool = False):
    ...
    def linalg_ldl_factor_ex_meta(hermitian: false, check_errors: false)
        square_check_inputs(self, "torch.linalg.ldl_factor_ex")
        check_floating_or_complex(self, "torch.linalg.ldl_factor_ex")
        ...
        pivots = self.new_empty(self.shape[0..-2], dtype: Torch.int)
        info = self.new_empty(self.shape[0..-3], dtype: Torch.int)
        [LD, pivots, info]
      end
      
      def linalg_ldl_solve_meta(LD, pivots, B, hermitian: false)
        square_check_inputs(LD, "torch.linalg.ldl_solve")
        check_floating_or_complex(LD, "torch.linalg.ldl_solve")
        linear_solve_check_inputs(B, LD, "torch.linalg.ldl_solve")
        torch._check(
          B.ndim >= 2,
          lambda: (
            "torch.linalg.ldl_solve: Expected B to have at least 2 dimensions, " \
            "but it has #{B.ndim} dimensions instead"
          )
        )
        expected_pivots_shape = LD.shape[0..-2]
        torch._check(
          expected_pivots_shape == pivots.shape,
          lambda: (
            "torch.linalg.ldl_solve: Expected LD.shape[0..-2] and pivots.shape to be the same, " \
            "but got pivots with shape #{pivots.shape} instead"
          )
        )
        torch._check(
          Utils.is_integer_dtype(pivots.dtype),
          lambda: "torch.linalg.ldl_solve: Expected pivots to be integers. Got #{pivots.dtype}"
        )
        torch._check(
          LD.dtype == B.dtype,
          lambda: "torch.linalg.ldl_solve: LD dtype #{LD.dtype} does not match b dtype #{B.dtype}"
        )
      ...
      def linalg_lu_meta(A, pivot: true)
        torch._check(
          A.ndim >= 2,
          lambda: "linalg.lu: Expected tensor with 2 or more dimensions. Got size: #{A.shape} instead"
        )
        ...
        sizes[-1] = m
        P = pivot ? A.new_empty(sizes) : A.new_empty([])
      ...
      def linalg_lu_factor_ex_meta(A, pivot: true, check_errors: false)
        torch._check(
          A.ndim >= 2,
          lambda: "torch.lu_factor: Expected tensor with 2 or more dimensions. Got size: #{A.shape} instead"
        )
      ...
      def linalg_lu_solve_meta(LU, pivots, B, left: true, adjoint: false)
        check_floating_or_complex(LU, "torch.linalg.lu_solve")
        torch._check(
          LU.dtype == B.dtype,
          lambda: "linalg.lu_solve: Expected LU and B to have the same dtype, " \
                  "but found LU of type #{LU.dtype} and B of type #{B.dtype} instead"
        )
        torch._check(
          pivots.dtype == Torch.int,
          lambda: "linalg.lu_solve: pivots should be a Tensor of scalar type torch.int32"
        )
        square_check_inputs(LU, "torch.linalg.lu_solve")
        check_inputs_solver(LU, B, left, "linalg.lu_solve")
        torch._check(
          LU.size[-1] == pivots.size[-1],
          lambda: "linalg.lu_solve: Number of pivots per batch should be same as the dimension of the matrix"
        )
        torch._check(
          LU.shape[0..-2] == pivots.shape,
          lambda: "linalg.lu_solve: Expected LU.shape[0..-2] and pivots.shape to be the same, " \
                  "but got pivots with shape #{pivots.shape} instead"
        )
        ...
        result = Torch.empty_strided(
          size: b_broadcast_size,
          stride: make_contiguous_strides_for(b_broadcast_size, row_major: !left),
          dtype: B.dtype,
          device: B.device
        )
      
        result.conj if !left && !result.empty? && result.complex?
...
def linalg_lu_solve_meta(LU, pivots, B, left: true, adjoint: false)
  ...
  torch._check(
    LU.dtype == B.dtype,
    lambda: (
      "linalg.lu_solve: Expected LU and B to have the same dtype, " \
      "but found LU of type #{LU.dtype} and B of type #{B.dtype} instead"
    )
  )
  torch._check(
  ...
  torch._check(
    LU.shape[0..-2] == pivots.shape,
    lambda: (
      "linalg.lu_solve: Expected LU.shape[0..-2] and pivots.shape to be the same, " \
      "but got pivots with shape #{pivots.shape} instead"
    )
  )
  B_broadcast_size, _ = _linalg_broadcast_batch_dims(B, LU)
...
def lu_unpack_meta(LU, pivots, unpack_data: true, unpack_pivots: true)
  torch._check(
    LU.ndim >= 2,
    lambda: "torch.lu_unpack: Expected tensor with 2 or more dimensions. Got size: #{LU.shape} instead"
  )
  if unpack_pivots
    torch._check(
  ...
  sizes[-1] = m
  P = unpack_pivots ? LU.new_empty(sizes) : LU.new_empty([0])
...
def _parse_qr_mode(mode: String)
  if mode == "reduced"
    compute_q = true
    reduced = true
  elsif mode == "complete"
    ...
    torch._check(
      false,
      lambda: (
        "qr received unrecognized mode '#{mode}' " \
...
def linalg_qr_meta(A, mode: "reduced")
  check_is_matrix(A, "linalg.qr")
  check_floating_or_complex(A, "linalg.qr")
  ...
  if compute_q
    Q_shape = A.shape.to_a
    Q_shape[-1] = reduced_mode ? k : m
  ...
  R_shape = A.shape.to_a
  R_shape[-2] = reduced_mode || !compute_q ? k : m
...
def _linalg_slogdet(A)
  ...
  check_floating_or_complex(A, "linalg.slogdet", false)
  shape = A.shape
  sign = A.new_empty(shape[0..-3])
  logabsdet = A.new_empty(shape[0..-3], dtype: to_real_value_type(A.dtype))
  LU = Torch.empty_strided(
    size: shape,
  ...
  pivots = A.new_empty(shape[0..-2], dtype: torch.int32)
...
def _linalg_svd_meta(
  ...
  check_floating_or_complex(A, "linalg.svd")

  batch_dims = A.shape[0..-3]
  m = A.shape[-2]
  n = A.shape[-1]
  k = [m, n].min

  if compute_uv
    U_shape = batch_dims + [m, full_matrices ? m : k]
    U = A.new_empty(U_shape)
    U.as_strided_(U_shape, make_contiguous_strides_for(U_shape, row_major: false))

    V_shape = batch_dims + [full_matrices ? n : k, n]
  ...
  S = A.new_empty(batch_dims + [k], dtype: to_real_value_type(A.dtype))
  [U, S, V]
end
...
def meta_embedding_bag_per_sample_weights_backward(grad, weight, indices, offsets, offset2bag, mode, padding_idx: -1)
  mode_sum, mode_mean, mode_max = 0, 1, 2
  embedding_features = grad.size(1)
  raise "embedding_bag_backward: per_sample_weights only supported for mode='sum'" unless mode == mode_sum
  raise "grad must have 2 dimensions" unless grad.dim() == 2
  raise "indices must have 1 dimension" unless indices.dim() == 1
  num_samples = indices.size(0)
  raise "weight must have 2 dimensions" unless weight.dim() == 2
  raise "weight's second dimension must match embedding_features" unless weight.size(1) == embedding_features
  output = torch.empty(num_samples)
  return output
end

def meta_isin(elements, test_elements, assume_unique: false, invert: false)
  unless elements.is_a?(Torch::Tensor) || test_elements.is_a?(Torch::Tensor)
    raise "At least one of elements and test_elements must be a Tensor."
  end

  elements = Torch.tensor(elements, device: test_elements.device) unless elements.is_a?(Torch::Tensor)
  test_elements = Torch.tensor(test_elements, device: elements.device) unless test_elements.is_a?(Torch::Tensor)

  _check_for_unsupported_isin_dtype(elements.dtype)
  _check_for_unsupported_isin_dtype(test_elements.dtype)
  return torch.empty_like(elements, dtype: :bool)
end

def meta_polygamma(n, self_tensor)
  raise "polygamma(n, x) does not support negative n." unless n >= 0
  _, result_dtype = elementwise_dtypes(self_tensor, type_promotion_kind: :int_to_float)
  return torch.empty_like(self_tensor, dtype: result_dtype)
end

def meta_channel_shuffle(input_tensor, groups)
  *leading_dims, c, h, w = input_tensor.size()
  return torch.empty(*leading_dims, c, h, w, dtype: input_tensor.dtype, layout: input_tensor.layout, device: input_tensor.device)
end

def _create_unary_float_meta_func(func_name)
  define_method(func_name) do |x|
    return elementwise_meta(x, type_promotion: :int_to_float)
  end
end

def _create_binary_float_meta_func(func_name)
  define_method(func_name) do |x, y|
    return elementwise_meta(x, y, type_promotion: :int_to_float)
  end
end
